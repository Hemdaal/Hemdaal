...
class ProjectCollaboratorRepository {
    ...
    fun getCollaboratorAccess(projectId: Long): Map<Collaborator, Access> {
        return transaction {
            ProjectCollaboratorTable.leftJoin(CollaboratorTable)
                ...
                .associate {
                    convertRowToCollaborator(it) to Access(createScopesFrom(it[ProjectCollaboratorTable.scopes]))
                }
        }
    }

    fun getProjectsWithScope(colId: Long): Map<Project, Access> {
        return transaction {
            ProjectCollaboratorTable.leftJoin(ProjectTable)
                ...
                .associate {
                    createProjectFrom(it) to Access(createScopesFrom(it[ProjectCollaboratorTable.scopes]))
                }
        }
    }

    fun getProjectWithScope(colId: Long, projectId: Long): Pair<Project, Access>? {
        return transaction {
            ProjectCollaboratorTable.leftJoin(ProjectTable)
